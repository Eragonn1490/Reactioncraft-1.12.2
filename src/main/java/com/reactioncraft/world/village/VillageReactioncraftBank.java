...
public class VillageReactioncraftBank extends StructureVillagePieces.House1 
{
	...
	@Nullable
	public static VillageReactioncraftBank buildComponent(StructureVillagePieces.Start startPiece, List<StructureComponent> pieces, Random random, int structureMinX, int structureMinY, int structureMinZ, EnumFacing facing, int componentType) {
		...
		return new VillageReactioncraftBank(startPiece, componentType, random, bbox, facing);
	}
	...
	public boolean addComponentParts(World world, Random random, StructureBoundingBox structBoundingBox) {
		...
		if (random.nextInt(2) == 0) {
			IBlockState Clayalizer = BlockIndex.clayalizerIdle.getDefaultState().withProperty(BlockClayalizer.FACING, EnumFacing.EAST);
